...
'''Single-Output (Porosity) Combine predictors using stacking
INSPIRED BY:  
...
lasso_pipeline = make_pipeline(linear_preprocessor, LassoCV(n_alphas = 50, max_iter = 500, eps = 0.0001, cv = 7))
...
rf_pipeline = make_pipeline(tree_preprocessor, RandomForestRegressor(random_state=42, n_estimators = 1400, min_samples_split = 2, min_samples_leaf = 1, max_features = 'auto', max_depth = 40, bootstrap = False))
print(rf_pipeline)

from sklearn.ensemble import GradientBoostingRegressor
gb_pipeline = make_pipeline(tree_preprocessor, GradientBoostingRegressor(n_estimators = 1000, min_samples_split = 5, min_samples_leaf = 1, max_depth = 90, loss = 'huber', learning_rate = 0.01, criterion = 'friedman_mse'))
print(gb_pipeline)
...
plt.savefig('stackedsinglepor1.png.png', dpi=300, bbox_inches='tight')
